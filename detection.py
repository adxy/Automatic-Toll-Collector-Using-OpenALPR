...
'''
def main(): #this is called from terminal only, for debugging purpose. {Syntax: python3 detection.py image_path.jpg}
...
def returnLP(plate_name): # This returns the LP when given with a image. (plate_name)
    ...
    try:
        ...
        alpr = Alpr('in', options.config, options.runtime_data)
        ...
        if not alpr.is_loaded():
            ...
            alpr.set_default_region("in")
            alpr.set_detect_region(False)
            jpeg_bytes = open(plate_name, "rb").read()
            results = alpr.recognize_array(jpeg_bytes)            
        ...
        if lp == None:
            return "Failed! Maybe try a clearer image!"
        else:
            return lp
...
def getLP(plate_name): # Call this to get the LP number.
    
    plate = returnLP(plate_name)
